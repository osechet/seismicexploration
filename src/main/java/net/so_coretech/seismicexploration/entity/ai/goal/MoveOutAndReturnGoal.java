    ...
    switch (phase) {
      case TO_START -> {
        if (!mob.blockPosition().closerThan(startPos, 1.2)) {
          ...
          phase = Phase.OUT;
          outTarget =
              getHighestBlock(this.mob.level(), mob.blockPosition().relative(direction, distance));
        }
      }
      case OUT -> {
        if (outTarget == null) {
        ...
        if (!mob.blockPosition().closerThan(outTarget, 1.2)) {
          LOGGER.trace(
              "At {}, moving out to target position: {} ({})",
              mob.blockPosition(),
              outTarget,
              mob.blockPosition().distSqr(outTarget));
          mob.getNavigation()
              .moveTo(outTarget.getX() + 0.5, outTarget.getY(), outTarget.getZ() + 0.5, 1.0);
        } else {
      ...
      case RETURN -> {
        if (!mob.blockPosition().closerThan(startPos, 1.2)) {
          LOGGER.trace(
              "At {}, returning to start position: {}({})",
              mob.blockPosition(),
              startPos,
              mob.blockPosition().distSqr(startPos));
          final BlockPos target = getHighestBlock(this.mob.level(), startPos);
